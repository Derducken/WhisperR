...
# --- Inlined hotkey_manager.py content ---
KEYSYM_MAP = {
...
class HotkeyRecorderDialog(tk.Toplevel):
    ...
    def _tkinter_key_press_callback(self, event: tk.Event):
        ...
        if key_name_normalized in ["esc", "enter"]: return
        if self._is_modifier(key_name_normalized):
            self.current_modifiers.add(key_name_normalized)
            if self.last_action_key:
                self.last_action_key = None; self.hotkey_string = None; self.ok_button.config(state=tk.DISABLED)
        else: 
            self.last_action_key = key_name_normalized
            combo_parts = sorted(list(self.current_modifiers)); combo_parts.append(self.last_action_key)
    ...
    def _tkinter_key_release_callback(self, event: tk.Event):
        key_name_normalized = self._normalize_key_name(event.keysym)
        if self._is_modifier(key_name_normalized):
            if key_name_normalized in self.current_modifiers: self.current_modifiers.remove(key_name_normalized)
            if not self.last_action_key and not self.current_modifiers:
                 self.hotkey_string = None; self.ok_button.config(state=tk.DISABLED)
        self._update_display_status()
    def _update_display_status(self):
        if self.hotkey_string: self.status_var.set(f"Recorded: {self.hotkey_string}")
        else: 
            display_parts = sorted(list(self.current_modifiers))
            if self.last_action_key: display_parts.append(self.last_action_key)
            if display_parts: self.status_var.set(f"Pressing: {'+'.join(display_parts)}")
            else: self.status_var.set("Press keys...")
            self.ok_button.config(state=tk.DISABLED)
    ...
    def _on_ok(self):
        if not self.hotkey_string or not self.last_action_key or self._is_modifier(self.last_action_key):
            ...
            self._start_recording_tkinter(); return
        self._stop_recording_tkinter(); self.destroy()
    def _on_cancel(self):
        self._stop_recording_tkinter(); self.hotkey_string = None; self.destroy()
    @staticmethod
    def record(parent) -> Optional[str]:
        dialog = HotkeyRecorderDialog(parent); return dialog.hotkey_string

class HotkeyManager:
    def __init__(self, root_tk_instance: tk.Tk):
        self.root = root_tk_instance; self._registered_hotkeys: Dict[str, Any] = {} 
        self.hotkey_toggle_record_cb: Optional[Callable] = None; self.hotkey_show_window_cb: Optional[Callable] = None
        self.ptt_pressed_cb: Optional[Callable[[], None]] = None; self.ptt_released_cb: Optional[Callable[[], None]] = None
        self.current_toggle_hk_str: Optional[str] = None; self.current_show_hk_str: Optional[str] = None
        self.current_ptt_hk_str: Optional[str] = None; self._ptt_hotkey_id: Optional[Any] = None
        self._ptt_release_hook_id: Optional[Any] = None; self._ptt_hotkey_pressed_flag = False
        self._parsed_ptt_main_key: Optional[str] = None
    def set_callbacks(self, toggle_record_cb: Optional[Callable], show_window_cb: Optional[Callable],
                      ptt_pressed_cb: Optional[Callable[[], None]] = None, ptt_released_cb: Optional[Callable[[], None]] = None):
        self.hotkey_toggle_record_cb = toggle_record_cb; self.hotkey_show_window_cb = show_window_cb
        self.ptt_pressed_cb = ptt_pressed_cb; self.ptt_released_cb = ptt_released_cb
    def update_hotkeys(self, toggle_record_str: Optional[str], show_window_str: Optional[str], hotkey_ptt_str: Optional[str]) -> bool:
        ...
        registration_errors = []
        if self.current_toggle_hk_str and self.hotkey_toggle_record_cb and not self._register_single_hotkey(self.current_toggle_hk_str, self.hotkey_toggle_record_cb):
            registration_errors.append(f"Toggle Record Hotkey: '{self.current_toggle_hk_str}'")
        if self.current_show_hk_str and self.hotkey_show_window_cb and not self._register_single_hotkey(self.current_show_hk_str, self.hotkey_show_window_cb):
            registration_errors.append(f"Show Window Hotkey: '{self.current_show_hk_str}'")
        if self.current_ptt_hk_str and self.ptt_pressed_cb and self.ptt_released_cb and not self._register_ptt_hotkey():
        ...
        log_essential(f"Hotkeys updated. Toggle: '{self.current_toggle_hk_str}', Show: '{self.current_show_hk_str}', PTT: '{self.current_ptt_hk_str}'"); return True
    def _register_single_hotkey(self, key_string: str, callback: Callable) -> bool:
        ...
        try: self._registered_hotkeys[key_string] = keyboard.add_hotkey(key_string, callback, suppress=False); log_essential(f"Registered hotkey: {key_string}"); return True
        except Exception as e: log_error(f"Failed to register hotkey '{key_string}': {e}"); return False
    def _parse_ptt_hotkey_main_key(self):
        ...
        try:
            ...
            for part in reversed(parts): 
                ...
                if not self._is_modifier(part_clean): self._parsed_ptt_main_key = part_clean; log_debug(f"Parsed PTT main key: {self._parsed_ptt_main_key} from {self.current_ptt_hk_str}"); return
            log_warning(f"Could not determine main action key for PTT hotkey: {self.current_ptt_hk_str}")
        except Exception as e: log_error(f"Error parsing PTT main key from '{self.current_ptt_hk_str}': {e}")
    def _is_modifier(self, normalized_key_name: str) -> bool: return normalized_key_name in {"ctrl", "alt", "shift", "win", "caps lock", "num lock"}
    def _register_ptt_hotkey(self) -> bool:
        if not self.current_ptt_hk_str or not self.ptt_pressed_cb or not self.ptt_released_cb: return False
        self._parse_ptt_hotkey_main_key()
        if not self._parsed_ptt_main_key: log_error(f"Cannot register PTT hotkey '{self.current_ptt_hk_str}': No main action key identified."); return False
        try: self._ptt_hotkey_id = keyboard.add_hotkey(self.current_ptt_hk_str, self._on_ptt_pressed_internal, suppress=False); log_essential(f"PTT hotkey press registered for '{self.current_ptt_hk_str}'."); return True
        except Exception as e: log_error(f"Failed to register PTT press hotkey '{self.current_ptt_hk_str}': {e}"); return False
    def _on_ptt_pressed_internal(self):
        if not self._ptt_hotkey_pressed_flag: 
            self._ptt_hotkey_pressed_flag = True; log_debug(f"PTT Pressed (internal): {self.current_ptt_hk_str}")
            if self.ptt_pressed_cb: self.ptt_pressed_cb()
            if self._parsed_ptt_main_key and not self._ptt_release_hook_id:
                try: self._ptt_release_hook_id = keyboard.on_release_key(self._parsed_ptt_main_key, self._on_ptt_released_internal, suppress=False); log_debug(f"PTT release hook registered for key '{self._parsed_ptt_main_key}'.")
                except Exception as e: log_error(f"Failed to register PTT release hook for '{self._parsed_ptt_main_key}': {e}")
    def _on_ptt_released_internal(self, event: Optional[keyboard.KeyboardEvent] = None): 
        if self._ptt_hotkey_pressed_flag and (event is None or (event and event.name == self._parsed_ptt_main_key)):
            self._ptt_hotkey_pressed_flag = False; log_debug(f"PTT Released (internal): Main key '{self._parsed_ptt_main_key}' was released (or forced).")
            if self.ptt_released_cb: self.ptt_released_cb()
            if self._ptt_release_hook_id:
                try: keyboard.unhook_key(self._ptt_release_hook_id); log_debug("PTT release hook unregistered.")
                except Exception as e: log_extended(f"Error unhooking PTT release hook: {e}")
    ...
    def _unregister_all_hotkeys(self):
        for key_string, hk_id in list(self._registered_hotkeys.items()):
            try: keyboard.remove_hotkey(hk_id); log_extended(f"Unregistered hotkey: {key_string}")
            except Exception as e: log_extended(f"Error unregistering hotkey '{key_string}': {e}")
        self._registered_hotkeys = {}
        if self._ptt_hotkey_id:
            try: keyboard.remove_hotkey(self._ptt_hotkey_id); log_extended(f"Unregistered PTT press hotkey: {self.current_ptt_hk_str}")
            except Exception as e: log_extended(f"Error unregistering PTT press hotkey '{self.current_ptt_hk_str}': {e}")
            self._ptt_hotkey_id = None
        if self._ptt_release_hook_id: 
            try: keyboard.unhook_key(self._ptt_release_hook_id); log_extended("Cleaned up PTT release hook.")
            except Exception as e: log_extended(f"Error cleaning up PTT release hook: {e}")
            self._ptt_release_hook_id = None
        self._ptt_hotkey_pressed_flag = False
    def cleanup(self): log_essential("Cleaning up hotkey manager..."); self._unregister_all_hotkeys()
    def record_new_hotkey_dialog_managed(self, parent_tk_window) -> Optional[str]:
        original_toggle_hk, original_show_hk, original_ptt_hk = self.current_toggle_hk_str, self.current_show_hk_str, self.current_ptt_hk_str
        self._unregister_all_hotkeys(); dialog_result = None
        try: dialog_result = HotkeyRecorderDialog.record(parent_tk_window)
        except Exception as e: log_error(f"Error during HotkeyRecorderDialog.record: {e}", exc_info=True)
        finally: self.update_hotkeys(original_toggle_hk, original_show_hk, original_ptt_hk)
        return dialog_result
    @staticmethod
    def record_new_hotkey(parent_tk_window) -> Optional[str]: return HotkeyRecorderDialog.record(parent_tk_window)
...
class WhisperRApp:
    def __init__(self, root: tk.Tk):
        ...
        self.transcription_service = TranscriptionService(
            settings_manager_instance=self.settings_manager, 
            root_tk_instance=self.root, 
            persistent_task_queue_ref=self.persistent_task_queue,
            app_asset_path_ref=self.app_asset_path 
        )
        
        self.audio_service = AudioService( # Use keywords that match AudioService's __init__
            settings_manager_ref=self.settings_manager, 
            root_tk_instance=self.root, 
            persistent_task_queue_ref=self.persistent_task_queue, 
            transcription_service_ref=self.transcription_service
        )
        ...
        self._temp_dirs_to_cleanup_on_exit: List[Path] = []
    ...
    def add_temp_dir_to_cleanup_on_exit(self, temp_dir_path: Path):
        if temp_dir_path and isinstance(temp_dir_path, Path) and temp_dir_path.is_dir():
            if temp_dir_path not in self._temp_dirs_to_cleanup_on_exit:
                self._temp_dirs_to_cleanup_on_exit.append(temp_dir_path)
                log_debug(f"Added temp directory for cleanup on exit: {temp_dir_path}")
        else:
            log_warning(f"Attempted to add invalid temp directory for cleanup: {temp_dir_path}")

    def _cleanup_registered_temp_dirs(self):
        if not self._temp_dirs_to_cleanup_on_exit:
            return
        
        log_essential("Attempting to clean up temporary download directories...")
        for temp_dir in list(self._temp_dirs_to_cleanup_on_exit): 
            log_debug(f"Cleaning up temporary directory: {temp_dir}")
            try:
                if temp_dir.exists() and temp_dir.is_dir(): 
                    shutil.rmtree(temp_dir, ignore_errors=False) 
                    log_extended(f"Successfully removed temporary directory: {temp_dir}")
                else:
                    log_debug(f"Temporary directory already removed or not found: {temp_dir}")
            except PermissionError as e_perm:
                 log_warning(f"PermissionError cleaning up temp_dir '{temp_dir}' (likely WinError 32, file still in use by OS briefly): {e_perm}. This is usually minor.")
            except Exception as e:
                log_error(f"Error cleaning up temporary directory '{temp_dir}': {e}", exc_info=True)
            
            if temp_dir in self._temp_dirs_to_cleanup_on_exit:
                self._temp_dirs_to_cleanup_on_exit.remove(temp_dir)
        log_essential("Finished attempting temporary directory cleanup.")
    ...
    def _connect_signals_and_slots(self):
        ...
        self.main_view.bind_model_change(self._handle_model_change) 
        ...
        self.main_view.add_menu_command("file", "Transcribe Audio File...", self._action_transcribe_manual_file) 
        self.main_view.add_menu_command("file", type="separator")
        ...
        if current_value != value:
            ...
            if setting_name == "ui_theme":
                ...
                if self.config_window and self.config_window.winfo_exists():
                    ...
                    if hasattr(self.config_window, '_apply_theme'): self.config_window._apply_theme()
    ...
    def _handle_model_change(self, new_model_name: str):
        log_debug(f"Model changed in UI to: {new_model_name}")
        self._update_setting_and_save('model', new_model_name) 
        
        if self.transcription_service and hasattr(self.transcription_service, 'prime_engine_model'):
            current_language = self.settings.language
            if not current_language:
                log_warning("Language not set, using 'auto' for model priming.")
                current_language = "auto"

            if hasattr(self.main_view, 'set_transient_status_message'):
                self.main_view.set_transient_status_message(f"Preparing model '{new_model_name}'...", duration_ms=0) 
            else:
                log_debug(f"UI Status: Preparing model '{new_model_name}'...")

            self.transcription_service.prime_engine_model(
                language=current_language,
                model_name=new_model_name,
                callback=self._handle_model_priming_result
            )
        else:
            log_error("Transcription service not available or does not support model priming.")

    def _handle_model_priming_result(self, success: bool, message: str):
        # CORRECTED: Changed log_info to log_debug
        log_debug(f"Model priming result: Success={success}, Message='{message}'") 
        
        ui_message = ""
        if success:
            ui_message = f"Model ready: {message.split('.')[0]}" 
            log_essential(f"Model priming successful: {message}")
        else:
            ui_message = f"Model prep failed: {message}"
            log_error(f"Model priming failed: {message}")
            
        if hasattr(self.main_view, 'set_transient_status_message'):
            self.main_view.set_transient_status_message(ui_message, duration_ms=7000) 
        else: 
            log_debug(f"UI Info: {ui_message}")
    ...
    def _action_quit_application(self):
        ...
        self._cleanup_registered_temp_dirs()
    ...
    def _handle_transcription_complete(self, transcribed_text: str, original_audio_path: Path):
        ...
        if self.settings.auto_paste and self.root.winfo_exists(): 
    ...
    def _action_transcribe_manual_file(self):
        ...
        try:
            ...
            shutil.copy2(selected_filepath, copied_filepath) 
    ...
    def _update_all_status_indicators(self):
        ...
        if self.status_bar_win_manager and self.status_bar_win_manager.enabled and self.root.winfo_exists():
            self.root.after(0, self.status_bar_win_manager.update_bar_color, current_color)
        if self.alt_status_indicator and self.alt_status_indicator.enabled and self.root.winfo_exists():
            self.root.after(0, self.alt_status_indicator.update_icon_by_state, current_icon_key)

def main():
    mutex = None 
    mutex_created_by_this_instance = False
    try:
        import win32event
        import win32api
        mutex_name = "Global\\WhisperR_App_Mutex_v2" 
        log_debug(f"Attempting to create or open mutex: {mutex_name}")
        
        mutex = win32event.CreateMutex(None, True, mutex_name) 
        last_error = win32api.GetLastError()
        
        if last_error == 183: 
            log_error("Another instance of WhisperR is already running (mutex exists). Terminating.")
            if mutex: win32api.CloseHandle(mutex) 
            sys.exit(1)
        elif mutex is None : 
            log_error(f"Failed to create or open mutex (handle is None), last_error: {last_error}. Allowing to run but multiple instances might occur.")
        else: 
            mutex_created_by_this_instance = True
            log_debug(f"Mutex '{mutex_name}' created successfully by this instance.")
            
    except ImportError:
        log_warning("win32event/win32api not found. Cannot check for multiple instances (this is normal on non-Windows).")
    except Exception as e_mutex:
        log_error(f"Error during mutex check: {e_mutex}. Allowing to run but multiple instances might occur.")

    try:
        from ctypes import windll
        windll.shcore.SetProcessDpiAwareness(1)
        log_debug("DPI awareness set (implies Windows).") 
    except Exception as e_dpi:
        log_debug(f"Could not set DPI awareness (non-Windows or error): {e_dpi}")
    ...
    try:
        ...
        if app_logger.LOGGER and hasattr(app_logger.LOGGER, 'close') and callable(app_logger.LOGGER.close):
            app_logger.LOGGER.close()
        
        if mutex_created_by_this_instance and mutex:
            try:
                import win32event 
                win32event.ReleaseMutex(mutex)
                log_debug("Mutex released by this instance.")
            except Exception as e_release_mutex:
                log_error(f"Error releasing mutex: {e_release_mutex}")
            finally:
                try:
                    import win32api 
                    win32api.CloseHandle(mutex)
                    log_debug("Mutex handle closed.")
                except Exception as e_close_mutex_handle:
                    log_error(f"Error closing mutex handle: {e_close_mutex_handle}")
