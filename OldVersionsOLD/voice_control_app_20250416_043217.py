...
class VoiceControlApp:
    ...
    def load_settings(self):
        ...
        try:
            if os.path.exists(CONFIG_FILE):
                with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                    ...
                    print(f"Core settings loaded from {CONFIG_FILE}")
            else:
                print(f"{CONFIG_FILE} not found, using core defaults.")
        except Exception as e:
            print(f"Error loading {CONFIG_FILE}: {e}. Using core defaults.")
        ...
        try:
            if os.path.exists(PROMPT_FILE):
                ...
                self.prompt = ""
                print(f"{PROMPT_FILE} not found, using empty prompt.")
        except Exception as e:
            self.prompt = ""
            print(f"Error loading {PROMPT_FILE}: {e}. Using empty prompt.")
            ...
            if os.path.exists(COMMANDS_FILE):
                with open(COMMANDS_FILE, 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(loaded, list):
                        ...
                        self.loaded_commands = []
                    print(f"Commands loaded from {COMMANDS_FILE}")
            else:
                self.loaded_commands = []
                print(f"{COMMANDS_FILE} not found, using empty commands.")
        except Exception as e:
            self.loaded_commands = []
            print(f"Error loading {COMMANDS_FILE}: {e}. Using empty commands.")
    ...
    def save_settings(self):
        ...
        try:
            ...
            print(f"Core settings saved to {CONFIG_FILE}")
        except Exception as e:
    ...
    def save_prompt(self):
        ...
        try:
            ...
            print(f"Error saving {PROMPT_FILE}: {e}")
    ...
    def save_commands(self):
        ...
        try:
            ...
            print(f"Error saving {COMMANDS_FILE}: {e}")
    ...
    def create_widgets(self):
        ...
        button_inner_frame.pack(anchor=tk.W)
    ...
    def update_prompt(self, event=None):
        ...
        if new_prompt != self.prompt:
            ...
            print(f"Prompt updated (length: {len(self.prompt)} chars)")
            self.save_prompt()
    ...
    def record_audio_continuously(self):
        ...
        def audio_callback(indata, frames, time_info, status):
            ...
            self.current_segment.append(indata.copy())  # Always append to current_segment

            if not self.command_mode:
                print("Command mode disabled, continuing to record without VAD.")
                return
        ...
        if filename:
            try:
                with open(filename, "r", encoding='utf-8') as f:
                    ...
                    self.prompt = content
                    self.save_prompt()
                print(f"Prompt imported from {filename}")
            except Exception as e:
