...
class VoiceControlApp:
    ...
    def setup_tray_icon_thread(self):
        try:
            ...
            self.log_message("essential", "Error: icon.png not found.")
        ...
        while True:
            try:
                ...
                if filepath is AUDIO_QUEUE_SENTINEL:
                    ...
                    break
                self.log_message("essential", f"Worker started processing: {filepath} at {datetime.datetime.now()}. Queue size: {self.transcription_queue.qsize()}")
                if os.path.exists(filepath) and not filepath.endswith(".transcribed"):
                    self.transcribe_audio(filepath)
                else:
                    self.log_message("extended", f"Worker skipping: {filepath} (file does not exist or already transcribed)")
                self.transcription_queue.task_done()
                self.log_message("extended", f"Finished processing {filepath}. Remaining queue size: {self.transcription_queue.qsize()}")
            except queue.Empty:
                self.log_message("extended", f"Transcription queue empty at {datetime.datetime.now()}. Waiting for new files...")
                continue
            except Exception as e:
                self.log_message("essential", f"Error in transcription worker for {filepath if 'filepath' in locals() else 'unknown file'}: {e}")
    ...
    def transcribe_audio(self, audio_path):
        ...
        try:
            ...
            if os.path.exists(expected_output_path_txt):
                self.log_message("essential", f"Transcription saved to {expected_output_path_txt}")
                ...
                self.log_message("essential", f"Whisper ran, but output file {expected_output_path_txt} not found.")
            ...
            if transcription_successful and os.path.exists(audio_path):
                try:
                    ...
                    self.log_message("extended", f"Renamed audio file to: {transcribed_path}")
                except Exception as e_mv:
                    self.log_message("essential", f"Error renaming audio file {audio_path}: {e_mv}")
            elif os.path.exists(audio_path):
                pass
        self.log_message("essential", f"Transcription task finished for: {audio_path}")
    ...
    def transcode_last_recording(self):
        self.log_message("essential", "Transcode last recording requested.")
        try:
            export_dir = getattr(self, 'export_folder', '.')
            if not os.path.isdir(export_dir):
                self.log_message("essential", f"Export dir not found: {export_dir}")
                return
            ...
            if not wav_files:
                self.log_message("extended", "No untranscribed recordings found.")
                return
            latest_recording = max(wav_files, key=os.path.getmtime)
            self.log_message("essential", f"Adding to queue for transcoding: {latest_recording}")
            self.transcription_queue.put(latest_recording)
        except Exception as e:
            self.log_message("essential", f"Error finding last recording: {e}")
    ...
    def create_backup(self, filepath):
        ...
        try:
            ...
            self.log_message("extended", f"Created backup: {backup_filename}")
        except Exception as e:
